...
Nodes SHALL be rendered as filled circles with a configurable diameter (default 10 pixels). Node color SHALL be determined by community assignment.

#### Scenario: Default node appearance
- **WHEN** a node is rendered with default settings
- **THEN** it appears as a filled circle of 10px diameter, colored by its community ID using a community color palette
...
Edges SHALL be rendered as lines between connected nodes. Intra-community edges SHALL be colored by community; inter-community edges SHALL be rendered in white or gray.
